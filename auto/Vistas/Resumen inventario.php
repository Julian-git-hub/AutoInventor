...
if(isset($_POST["consulta"]))
{
    $c = new Conexion();
    $cone = $c->conectando();
    $sql = "select * from resumen_inventario where Entrada like '%$obj->Entrada%'";
    ...
    $c = new Conexion();
    $cone = $c->conectando();
    $sql = "select * from resumen_inventario";
    $limite =sprintf("%s limit %d, %d",$sql, $inicia, $maximoDatos);
    $resultado = mysqli_query($cone,$limite);
    $arreglo = mysqli_fetch_row($resultado);
}
...
<body>
    <h1 style="text-align: center;">Resumen Inventario</h1>
    <form action="" method="POST">
        <div class="consultas">
            <label for="" style="margin-right: 5px;">Consultar</label>
            <input type="text" id="Modelo" name="Modelo" style="width: 99%; " placeholder="Digite su consulta">
            <br><br>
            <button type="submit" name="consulta">Buscar</button>
            
            <a href="agregarmarca.php"><button type="button">Agregar Marca</button></a>
                    ...
                    <tr>
                        ...
                        <td class="letra">
                            <a href="<?php if($arreglo[0] <> ""){
                                echo "modificarmarca.php?key=".urlencode($arreglo[0]);
                            }else{
                        ...
                        <td class="letra">
                            <a href="<?php if($arreglo[0]<>""){
                                        echo "eliminarmarca.php?key=".urlencode($arreglo[0]);
                            }else{
                                echo "<script> alert('debe de consultar primero')</sccript>";
                            }
                                ?>">
