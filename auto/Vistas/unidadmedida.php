...
if(isset($_POST["consulta"]))
{
    $c = new Conexion();
    $cone = $c->conectando();
    $sql = "select * from unidad_medida where tipo_UND like '%$obj->tipo_UND%'";
...
<body>
    <h1 style="text-align: center;">Agregar unidad de medida</h1>
    <form action="" method="POST">
        <div class="consultas">
            <label for="" style="margin-right: 5px;">Consultar</label>
            <input type="text" id="tipo_UND" name="tipo_UND" style="width: 99%; " placeholder="Digite su consulta">
            <br><br>
            <button type="submit" name="consulta">Buscar</button>
            <a href="agregarunidadmedida.php"><button type="button">Agregar Unidad de Medida</button></a>
        </div>
        <br>
        <div class="formM">
            <table class="tabla">    
                    ...
                    <tr>
                        ...
                        <td class="letra">
                            <a href="<?php if($arreglo[0] <> ""){
                                echo "modificarunidadmedida.php?key=".urlencode($arreglo[0]);
                            }else{
                        ...
                        <td class="letra">
                            <a href="<?php if($arreglo[0]<>""){
                                        echo "eliminarunidadmedida.php?key=".urlencode($arreglo[0]);
                            }else{
                                echo "<script> alert('debe de consultar primero')</sccript>";
                            }
                                ?>">
