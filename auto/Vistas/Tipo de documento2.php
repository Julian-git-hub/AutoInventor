...
if(isset($_POST["consulta"]))
{
    $c = new Conexion();
    $cone = $c->conectando();
    $sql = "select * from tipo_documento_con where Descripcion like '%$obj->Descripcion%'";
            /*"select tipo_documento_con.ID_tipodocumento,tipo_documento_con.Descripcion,tipo_movimiento.Descripcion 
            from tipo_documento_con INNER JOIN tipo_movimiento on tipo_documento_con.ID_tipomovimiento = tipo_movimiento.ID_tipomovimiento 
            where Descripcion like '%$obj->Descripcion%'";*/
    ...
    $c = new Conexion();
    $cone = $c->conectando();
    $sql = "select tipo_documento_con.ID_tipodocumento,tipo_documento_con.Descripcion,tipo_movimiento.Descripcion 
            from tipo_documento_con INNER JOIN tipo_movimiento on tipo_documento_con.ID_tipomovimiento = tipo_movimiento.ID_tipomovimiento";
    $limite =sprintf("%s limit %d, %d",$sql, $inicia, $maximoDatos);
    $resultado = mysqli_query($cone,$limite);
    $arreglo = mysqli_fetch_row($resultado);
...
<body>
    <h1 style="text-align: center;">Tipo de Documento contable</h1>
    <form action="" method="POST">
        <div class="consultas">
            ...
            <a href="agregarTipodocumento2.php"><button type="button">Agregar Tipo de Documento contable</button></a>
            <br>
        </div>
        <br>
            <div class="formM">
                    ...
                    <tr>
                        ...
                        <td class="letra">
                            <a href="<?php if($arreglo[0] <> ""){
                                echo "modificarTipoDocumento2.php?key=".urlencode($arreglo[0]);
                            }else{
                        ...
                        <td class="letra">
                            <a href="<?php if($arreglo[0]<>""){
                                        echo "eliminarTipoDocumento2.php?key=".urlencode($arreglo[0]);
                            }else{
                                echo "<script> alert('debe de consultar primero')</sccript>";
                            }
                                ?>">
