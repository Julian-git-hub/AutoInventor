...
if(isset($_POST["consulta"]))
{
    $c = new Conexion();
    $cone = $c->conectando();
    $sql = "select * from modulos where Descripcion like '%$obj->Descripcion%'";
    ...
    $c = new Conexion();
    $cone = $c->conectando();
    $sql = "SELECT modulos.ID_modulos, modulos.Descripcion,permisos.Descripcion FROM `modulos` INNER JOIN permisos on modulos.ID_permisos = permisos.ID_permisos";
    $limite =sprintf("%s limit %d, %d",$sql, $inicia, $maximoDatos);
    $resultado = mysqli_query($cone,$limite);
    $arreglo = mysqli_fetch_row($resultado);
}
...
<body>
    ...
    <form action="" method="POST">
        <div class="consultas">
            ...
            <a href="agregarmodulo.php"><button type="button">Agregar Terceros</button></a>
            <br>
        </div>
        <br>
            <div class="formM">
                    ...
                    <tr>
                        ...
                        <td class="letra">
                            <a href="<?php if($arreglo[0] <> ""){
                                echo "modificarmodulo.php?key=".urlencode($arreglo[0]);
                            }else{
                        ...
                        <td class="letra">
                            <a href="<?php if($arreglo[0]<>""){
                                        echo "eliminarModulo.php?key=".urlencode($arreglo[0]);
                            }else{
                                echo "<script> alert('debe de consultar primero')</sccript>";
                            }
                                ?>">
