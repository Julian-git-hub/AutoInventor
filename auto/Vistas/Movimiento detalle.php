...
if(isset($_POST["consulta"]))
{
    $c = new Conexion();
    $cone = $c->conectando();
    $sql = "select * from movimiento_detalle where Cantidad like '%$obj->Cantidad%'";
    ...
    $c = new Conexion();
    $cone = $c->conectando();
    $sql = "select * from movimiento_detalle";
    $limite =sprintf("%s limit %d, %d",$sql, $inicia, $maximoDatos);
    $resultado = mysqli_query($cone,$limite);
    $arreglo = mysqli_fetch_row($resultado);
}
...
<body>
    <h1  style="text-align: center;">Movimiento detalle</h1>
    <form action="" method="POST">
        <div class="consultas">
            <label for="" style="margin-right: 5px;">Movimiento Cabecera</label>
            <input type="text" id="Cantidad" name="Cantidad" style="width: 99%;" placeholder="Digite su consulta">
            <br><br>
            <button type="submit" name="consulta">Buscar</button>
            <a href="agregarmovimientodetalle.php"><button type="button">Agregar movimiento Detalle</button></a>
            <br>
        </div>
        <br>
            <div class="formM">
                    ...
                    <tr>
                        ...
                        <td class="letra">
                            <a href="<?php if($arreglo[0] <> ""){
                                echo "modificarmovimientodetalle.php?key=".urlencode($arreglo[0]);
                            }else{
                        ...
                        <td class="letra">
                            <a href="<?php if($arreglo[0]<>""){
                                        echo "eliminarmovimientodetalle.php?key=".urlencode($arreglo[0]);
                            }else{
                                echo "<script> alert('debe de consultar primero')</sccript>";
                            }
                                ?>">
